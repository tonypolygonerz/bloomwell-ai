  ...
  try {
    ...
    for (let i = 0; i < guidelinesData.length; i++) {
      ...
      try {
        ...
        console.log(
          `✅ [${i + 1}/${guidelinesData.length}] Created: ${createdGuideline.name} (Priority: ${createdGuideline.priority})`
        );
      } catch (error) {
        console.error(
          `❌ Failed to create guideline "${guidelineData.name}":`,
          error
        );
        throw error;
      }
