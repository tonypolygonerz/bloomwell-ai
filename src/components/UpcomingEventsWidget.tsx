...
export default function UpcomingEventsWidget({ userId }: UpcomingEventsWidgetProps) {
  ...
  const upcomingEvents = userRSVPs.filter(rsvp => 
    isAfter(new Date(rsvp.webinar.scheduledDate), new Date())
  ).sort((a, b) => 
    new Date(a.webinar.scheduledDate).getTime() - new Date(b.webinar.scheduledDate).getTime()
  )
    ...
    return (
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="text-center py-8">
          ...
          <h4 className="mt-2 text-sm font-medium text-gray-900">No upcoming events</h4>
    ...
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      <div className="space-y-4">
        {upcomingEvents.slice(0, 3).map((rsvp) => {
          ...
          return (
            <div key={rsvp.id} className="border-b border-gray-200 last:border-b-0 pb-4 last:pb-0">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {format(new Date(rsvp.webinar.scheduledDate), 'MMM d, yyyy • h:mm a')}
                  </p>
                  <div className="flex items-center mt-2">
                    <span className={`text-sm font-medium ${getStatusColor(timeInfo.status)}`}>
                      {timeInfo.text}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="flex space-x-1">
                    <button
                      onClick={() => {
                        ...
                        const endDate = new Date(startDate.getTime() + rsvp.webinar.duration * 60 * 1000)
