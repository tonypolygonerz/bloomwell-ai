...
export default function OrganizationSearchField({
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      try {
        const response = await fetch(`/api/organizations/search?q=${encodeURIComponent(value)}`);
        if (response.ok) {
  ...
  return (
    <div className="space-y-2" ref={dropdownRef}>
      ...
      <p className="text-xs text-gray-500">Start typing to search IRS-registered nonprofits</p>
      ...
      {showDropdown && value.length >= 3 && (
        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-80 overflow-y-auto">
          {isSearching ? (
            ...
            <div className="p-4">
              <p className="text-sm text-gray-600 mb-3">No organizations found</p>
              <button
                type="button"
                onClick={handleManualEntry}
      ...
      {showManualEntry && (
        <div className="mt-4 p-4 bg-gray-50 rounded-lg space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <p className="text-sm font-medium text-gray-900">Enter organization details</p>
          <div>
            <label htmlFor="manual-name" className="block text-xs font-medium text-gray-700 mb-1">
              Organization name <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="manual-ein" className="block text-xs font-medium text-gray-700 mb-1">
              EIN (optional)
            </label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="manual-city" className="block text-xs font-medium text-gray-700 mb-1">
                City
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="manual-state" className="block text-xs font-medium text-gray-700 mb-1">
                State
              </label>
              <input
                type="text"
