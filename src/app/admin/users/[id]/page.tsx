...
export default function UserDetail() {
  ...
  const getAccountTypeColor = (type: string) => {
    switch (type.toLowerCase()) {
      ...
      case 'azure-ad': return 'bg-green-100 text-green-800'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
              ...
              <div className="px-4 py-5 sm:p-6">
                ...
                <dl className="space-y-4">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{formatDate(user.lastLogin)}</dd>
                  </div>
                  
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Total Chats</dt>
                    <dd className="mt-1 text-sm text-gray-900">{user.conversationCount}</dd>
                  </div>
                  
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Webinar RSVPs</dt>
                    <dd className="mt-1 text-sm text-gray-900">{user.rsvpCount}</dd>
                  </div>
                </dl>
              </div>
            </div>
            ...
            {user.organization && (
              <div className="bg-white shadow rounded-lg mt-6">
                <div className="px-4 py-5 sm:p-6">
                  <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Organization</h3>
                  ...
                  <dl className="space-y-4">
                    ...
                    {user.organization.mission && (
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900">{user.organization.mission}</dd>
                      </div>
                    )}
                    ...
                    {user.organization.budget && (
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900">{user.organization.budget}</dd>
                      </div>
                    )}
                    
                    {user.organization.staffSize && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">Staff Size</dt>
                        <dd className="mt-1 text-sm text-gray-900">{user.organization.staffSize}</dd>
                      </div>
                    )}
                    ...
                    {user.organization.focusAreas && (
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900">{user.organization.focusAreas}</dd>
                      </div>
                    )}
                  </dl>
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white shadow rounded-lg">
              <div className="px-4 py-5 sm:p-6">
                <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Webinar RSVPs</h3>
                
                {rsvps.length === 0 ? (
                  <p className="text-sm text-gray-500">No webinar RSVPs found.</p>
                ) : (
                  <div className="space-y-4">
                    {rsvps.map((rsvp) => (
                      <div key={rsvp.id} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="text-sm font-medium text-gray-900">{rsvp.webinar.title}</h4>
                            <p className="text-sm text-gray-500">
                              Scheduled: {formatDate(rsvp.webinar.scheduledDate)}
                            </p>
                            <p className="text-sm text-gray-500">
                          ...
                          <div className="text-right">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(rsvp.webinar.status)}`}>
                              {rsvp.webinar.status}
                            </span>
