...
export async function generateMetadata({
  ...
  const imageUrl =
    webinar.socialImageUrl ||
    webinar.thumbnailUrl ||
    `${baseUrl}/images/webinar-default.jpg`;
  ...
  return {
    ...
    description:
      webinar.metaDescription || webinar.description.substring(0, 160),
    keywords: webinar.categories
      ? webinar.categories.join(', ')
      : 'webinar, nonprofit, training, education',
...
export default async function WebinarPage({ params }: WebinarPageProps) {
  ...
  const structuredData = {
    ...
    location: {
      ...
      url:
        webinar.jitsiRoomUrl ||
        `${process.env.NEXTAUTH_URL}/webinar/${webinar.slug || webinar.uniqueSlug}`,
    },
    organizer: {
      '@type': 'Organization',
      name: 'Bloomwell AI',
